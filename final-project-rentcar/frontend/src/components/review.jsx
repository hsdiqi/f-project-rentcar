    ...
    <nav id="Review" className="review">
      {reviews.slice(0, 4).map((review, index) => (
        <div key={index} className="card card-custom">
          <div className="sec">
            <div>
              <img className="card-img-top" src={`/assets/${review.FOTO_PELANGGAN}`} alt={"foto "+review.NAMA_PELANGGAN} />
            </div>
            <div>
              <h3><label htmlFor="name">{review.NAMA_PELANGGAN}</label></h3>
              <h5><label htmlFor="Member">{review.JENIS_MEMBERSHIP}</label></h5>
            </div>
          </div>
